...
const KEY = 'booksDB'
const booksFromStorage = storageService.loadFromStorage(KEY)
const gBooks = (booksFromStorage && booksFromStorage.length) ? booksFromStorage : getBooks()
...
function removeReview(book, reviewId) {
    let reviewIdx = book.reviews.findIndex(function(review) {
...
function getGoogleBook(googleBook) {
    ...
    return fetch(url)
        ...
        .then(res => {
            ...
            storageService.saveToStorage(googleBook.toLowerCase(), books)
            return books
        })
        .catch((err) => {
